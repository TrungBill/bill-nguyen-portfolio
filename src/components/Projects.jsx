...
const ScrollReveal = ({ children, delay = 0 }) => {
  return (
    <motion.div
      ...
      viewport={{ once: false, amount: 0.3 }} // Lower threshold - only 30% needs to be visible
    ...
    <ScrollReveal delay={index * 0.2}>
      <div className='flex flex-col items-center gap-8 md:flex-row md:gap-24'>
        <img 
          src={project.image} 
          alt={project.title} 
          className='w-full cursor-pointer rounded-2xl transition-all duration-300 hover:scale-105 md:w-[250px]'
        />
        <div className='flex flex-col gap-5'>
          ...
          <div className='flex flex-wrap gap-5'>
            {project.technologies.map((tech, index) => (
              <span key={index} className='rounded-lg bg-purple-600/30 p-3'>
                {tech}
              </span>
            ))}
          </div>
...
const Projects = () => {
  return (
    <div id="projects" className="mb-32 mt-32 min-h-screen w-full py-20">
      <div className="container mx-auto flex flex-col items-center justify-center gap-24 p-4">
        <ScrollReveal>
          <h1 className='text-4xl font-light text-white md:text-6xl'>Projects</h1>
        </ScrollReveal>
       
        <div className="flex w-full max-w-[1000px] flex-col gap-32 text-white">
